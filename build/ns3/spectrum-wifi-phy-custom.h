...
#ifndef SPECTRUM_WIFI_PHY_CUSTOM_H
#define SPECTRUM_WIFI_PHY_CUSTOM_H
 ...
 * \ingroup wifi
 *
 * This PHY implements a spectrum-aware enhancement of the 802.11 SpectrumWifiPhyCustom
 * model.
 *
 ...
 *
 */
class SpectrumWifiPhyCustom : public WifiPhy
{
public:
  static TypeId GetTypeId (void);

  SpectrumWifiPhyCustom ();
  virtual ~SpectrumWifiPhyCustom ();

  /**
   * Set the SpectrumChannel this SpectrumWifiPhyCustom is to be connected to.
   *
   * \param channel the SpectrumChannel this SpectrumWifiPhyCustom is to be connected to
   */
   ...
   * and low-level Phy interface to this SpectrumWifiPhyCustom instance.
   *
   ...
   * to link it to this SpectrumWifiPhyCustom instance
   *
   ...
   */
  Ptr<WifiSpectrumPhyCustomInterface> GetSpectrumPhy (void) const;
  /**
   ...
   * SpectrumWifiPhyCustom wish to adjust signal properties based on the
...
private:
  ...
  Ptr<SpectrumChannel> m_channel;        //!< SpectrumChannel that this SpectrumWifiPhyCustom is connected to
  std::vector<uint16_t> m_operationalChannelList; //!< List of possible channels

  Ptr<WifiSpectrumPhyCustomInterface> m_wifiSpectrumPhyInterface;
  Ptr<AntennaModel> m_antenna;
  mutable Ptr<const SpectrumModel> m_rxSpectrumModel;
  RxCallback m_rxCallback;
  bool m_disableWifiReception;          //!< forces this Phy to fail to sync on any signal
  TracedCallback<bool, uint32_t, double, Time> m_signalCb;
  TracedCallback<int, int, double, double, double, bool> m_tracedUpdateLinkQuality;
  void NotifyUpdateLinkQuality(Ptr<Packet> packet, int nodeId, double snr, double sinr, double per, bool received);

};

} //namespace ns3

#endif /* SPECTRUM_WIFI_PHY_CUSTOM_H */
