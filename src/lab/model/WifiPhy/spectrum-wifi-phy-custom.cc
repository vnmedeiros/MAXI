...
NS_LOG_COMPONENT_DEFINE ("SpectrumWifiPhyCustom");

NS_OBJECT_ENSURE_REGISTERED (SpectrumWifiPhyCustom);

TypeId
SpectrumWifiPhyCustom::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SpectrumWifiPhyCustom")
    .SetParent<WifiPhy> ()
    .SetGroupName ("Wifi")
    .AddConstructor<SpectrumWifiPhyCustom> ()
    .AddAttribute ("DisableWifiReception", "Prevent Wi-Fi frame sync from ever happening",
                   BooleanValue (false),
                   MakeBooleanAccessor (&SpectrumWifiPhyCustom::m_disableWifiReception),
                   MakeBooleanChecker ())
    .AddTraceSource ("SignalArrival",
                     "Signal arrival",
                     MakeTraceSourceAccessor (&SpectrumWifiPhyCustom::m_signalCb),
                     "ns3::SpectrumWifiPhyCustom::SignalArrivalCallback")
	.AddTraceSource ("UpdateLinkQuality",
					 "UpdateLinkQuality",
					 MakeTraceSourceAccessor (&SpectrumWifiPhyCustom::m_tracedUpdateLinkQuality),
					 "ns3::SpectrumWifiPhyCustom::UpdateLinkQualityCallback")
  ;
  return tid;
}

SpectrumWifiPhyCustom::SpectrumWifiPhyCustom ()
{
...
void
SpectrumWifiPhyCustom::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_channel = 0;
...
void
SpectrumWifiPhyCustom::DoInitialize (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_channel && m_wifiSpectrumPhyInterface)
    ...
    {
      NS_FATAL_ERROR ("SpectrumWifiPhyCustom misses channel and WifiSpectrumPhyInterface objects at initialization time");
    }
}

bool
SpectrumWifiPhyCustom::DoChannelSwitch (uint16_t nch)
{
  ...
  switch (m_state->GetState ())
    {
    case SpectrumWifiPhyCustom::RX:
      NS_LOG_DEBUG ("drop packet because of channel switching while reception");
      m_endPlcpRxEvent.Cancel ();
      m_endRxEvent.Cancel ();
      goto switchChannel;
      break;
    case SpectrumWifiPhyCustom::TX:
      NS_LOG_DEBUG ("channel switching postponed until end of current transmission");
      Simulator::Schedule (GetDelayUntilIdle (), &WifiPhy::SetChannelNumber, this, nch);
      break;
    case SpectrumWifiPhyCustom::CCA_BUSY:
    case SpectrumWifiPhyCustom::IDLE:
      goto switchChannel;
      break;
    case SpectrumWifiPhyCustom::SLEEP:
...
bool
SpectrumWifiPhyCustom::DoFrequencySwitch (uint32_t frequency)
{
  if (IsInitialized () == false)
    {
      //this is not channel switch, this is initialization
...
SpectrumWifiPhyCustom::GetRxSpectrumModel () const
{
  NS_LOG_FUNCTION (this);
...
Ptr<WifiChannel>
SpectrumWifiPhyCustom::GetChannel (void) const
{
  return Ptr<WifiChannel> (0);
}

void
SpectrumWifiPhyCustom::SetChannel (Ptr<SpectrumChannel> channel)
{
  m_channel = channel;
}

void
SpectrumWifiPhyCustom::SetPacketReceivedCallback (RxCallback callback)
{
  m_rxCallback = callback;
}

void
SpectrumWifiPhyCustom::AddOperationalChannel (uint16_t channelNumber)
{
  m_operationalChannelList.push_back (channelNumber);
}

std::vector<uint16_t>
SpectrumWifiPhyCustom::GetOperationalChannelList () const
{
...
void
SpectrumWifiPhyCustom::ClearOperationalChannelList ()
{
  m_operationalChannelList.clear ();
}

void
SpectrumWifiPhyCustom::SetSleepMode (void)
{
  NS_LOG_FUNCTION (this);
  switch (m_state->GetState ())
    {
    case SpectrumWifiPhyCustom::TX:
      ...
      Simulator::Schedule (GetDelayUntilIdle (), &SpectrumWifiPhyCustom::SetSleepMode, this);
      break;
    case SpectrumWifiPhyCustom::RX:
      NS_LOG_DEBUG ("setting sleep mode postponed until end of current reception");
      Simulator::Schedule (GetDelayUntilIdle (), &SpectrumWifiPhyCustom::SetSleepMode, this);
      break;
    case SpectrumWifiPhyCustom::SWITCHING:
...
void
SpectrumWifiPhyCustom::ResumeFromSleep (void)
{
  NS_LOG_FUNCTION (this);
  switch (m_state->GetState ())
    {
...
void
SpectrumWifiPhyCustom::SetReceiveOkCallback (RxOkCallback callback)
{
  m_state->SetReceiveOkCallback (callback);
}

void
SpectrumWifiPhyCustom::SetReceiveErrorCallback (RxErrorCallback callback)
{
  m_state->SetReceiveErrorCallback (callback);
}

void
SpectrumWifiPhyCustom::SwitchMaybeToCcaBusy (void)
{
  NS_LOG_FUNCTION (this);
...
void
SpectrumWifiPhyCustom::StartRx (Ptr<SpectrumSignalParameters> rxParams)
{
  ...
  NS_LOG_DEBUG ("Received signal from " << senderNodeId << " with unfiltered power " << WToDbm (Integral (*receivedSignalPsd)) << " dBm for " << this->GetDevice()->GetNode()->GetId());
//  std::cout << "Received signal from " << senderNodeId << " with unfiltered power " << WToDbm (Integral (*receivedSignalPsd)) << " dBm for " << this->GetDevice()->GetNode()->GetId() << std::endl;
  ...
  if (txVector.GetNss () > GetNumberOfReceiveAntennas ())
    {
      /* failure. */
      NotifyRxDrop (packet); //TODO falha de TX
      ...
      if (rxPowerW > GetEdThresholdW ()) //checked here, no need to check in the payload reception (current implementation assumes constant rx power over the packet duration)
        {
          ...
          if (preamble != WIFI_PREAMBLE_NONE)
            {
              NS_ASSERT (m_endPlcpRxEvent.IsExpired ());
              m_endPlcpRxEvent = Simulator::Schedule (preambleAndHeaderDuration, &SpectrumWifiPhyCustom::StartReceivePacket, this,
                                                      packet, txVector, preamble, mpdutype, event);
            }

          NS_ASSERT (m_endRxEvent.IsExpired ());
          m_endRxEvent = Simulator::Schedule (rxDuration, &SpectrumWifiPhyCustom::EndReceive, this,
...
void
SpectrumWifiPhyCustom::StartReceivePacket (Ptr<Packet> packet,
  ...
  if (m_random->GetValue () > snrPer.per) //plcp reception succeeded
    {
      if (IsModeSupported (txMode) || IsMcsSupported (txMode))
      ...
      else //mode is not allowed
        {
          ...
          NotifyRxDrop (packet); //TODO perda
          m_plcpSuccess = false;
        }
    }
...
Ptr<WifiSpectrumPhyCustomInterface>
SpectrumWifiPhyCustom::GetSpectrumPhy (void) const
{
  return m_wifiSpectrumPhyInterface;
}

Ptr<AntennaModel>
SpectrumWifiPhyCustom::GetRxAntenna (void) const
{
  return m_antenna;
}

void
SpectrumWifiPhyCustom::SetAntenna (Ptr<AntennaModel> a)
{
...
void
SpectrumWifiPhyCustom::CreateWifiSpectrumPhyInterface (Ptr<NetDevice> device)
{
  NS_LOG_FUNCTION (this << device);
  m_wifiSpectrumPhyInterface = CreateObject<WifiSpectrumPhyCustomInterface> ();
  m_wifiSpectrumPhyInterface->SetSpectrumWifiPhyCustom (this);
  m_wifiSpectrumPhyInterface->SetDevice (device);
}

void
SpectrumWifiPhyCustom::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble)
{
  SendPacket (packet, txVector, preamble, NORMAL_MPDU);
}

Ptr<SpectrumValue>
SpectrumWifiPhyCustom::GetTxPowerSpectralDensity (uint32_t centerFrequency, uint32_t channelWidth, double txPowerW) const
{
...
void
SpectrumWifiPhyCustom::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble, enum mpduType mpdutype)
{
...
void
SpectrumWifiPhyCustom::RegisterListener (WifiPhyListener *listener)
{
  m_state->RegisterListener (listener);
}

void
SpectrumWifiPhyCustom::UnregisterListener (WifiPhyListener *listener)
{
  m_state->UnregisterListener (listener);
}

void
SpectrumWifiPhyCustom::EndReceive (Ptr<Packet> packet, enum WifiPreamble preamble, enum mpduType mpdutype, Ptr<InterferenceHelper::Event> event)
{
  ...
  double sinr = RatioToDb(snrPer.snr);
  double snr = signalNoise.signal - signalNoise.noise;

	if (m_plcpSuccess == true) {
		NS_LOG_DEBUG("mode=" << (event->GetPayloadMode ().GetDataRate (event->GetTxVector ())) << ", snr(dB)=" << RatioToDb (snrPer.snr) << ", per=" << snrPer.per << ", size=" << packet->GetSize ());
		...
		if (m_random->GetValue () > snrPer.per) {
			...
			this->NotifyUpdateLinkQuality(packet, this->GetDevice()->GetNode()->GetId(), snr, sinr, snrPer.per, true);
			NotifyMonitorSniffRx(packet, (uint16_t) GetFrequency(),
					GetChannelNumber(), dataRate500KbpsUnits,
					event->GetPreambleType(), event->GetTxVector(), aMpdu,
					signalNoise);
			...
			/* failure. */
			//this->NotifyUpdateLinkQuality(packet,this->GetDevice()->GetNode()->GetId(), snr, sinr, false);
			this->NotifyUpdateLinkQuality(packet, this->GetDevice()->GetNode()->GetId(), snr, sinr, snrPer.per, false);
...
void SpectrumWifiPhyCustom::NotifyUpdateLinkQuality(Ptr<Packet> packet, int nodeId, double snr, double sinr, double per, bool received) {
	InfoTag it;
	if (packet->PeekPacketTag(it) ) {
		m_tracedUpdateLinkQuality( it.GetSrc(), nodeId, snr, sinr, per, received);
	}
}
