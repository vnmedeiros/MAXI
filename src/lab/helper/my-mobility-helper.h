...
#ifndef MY_MOBILITY_HELPER_H
#define MY_MOBILITY_HELPER_H
 ...
 *
 * MyMobilityHelper::Install is the most important method here.
 */
class MyMobilityHelper
{
public:
  /**
   ...
   * \param v9 the value of the attribute to set in the mobility model.
   *
   * Calls to MyMobilityHelper::Install will create an instance of a matching 
   ...
   *
   * If this stack is not empty when MyMobilityHelper::Install
   ...
   * type of which was set with MyMobilityHelper::SetMobilityModel), aggregates
   * it to the provided node, and sets an initial position based on the current
   * position allocator (set through MyMobilityHelper::SetPositionAllocator). 
   *
   * \param node The node to "layout."
   */
  bool Install (Ptr<Node> node) const;
  /**
   * \brief "Layout" a single node according to the current position allocator type.
   *
   * This method creates an instance of a ns3::MobilityModel subclass (the 
   ...
   * MyMobilityHelper::SetMobilityModel), aggregates it to the node, and sets an 
   * initial position based on the current position allocator (set through 
   * MyMobilityHelper::SetPositionAllocator). 
   *
  ...
  /**
   * Perform the work of MyMobilityHelper::Install on _all_ nodes which
  ...
  void SetNNodes (int nNodes); //Set the number of nodes
  void SetMinDist (double minDist); //Set the minimum distance between nodes
  void SetMaxDist (double maxDist);//Set the maximum distance between nodes
  Vector *GetAllocatedPositions (); //Get allocated positions for all nodes
  int *GetMatrix (); //Get the connectivity matrix
...
private:
  ...
  double m_minDist; //Minimum distance between nodes
  double m_maxDist; //Maximum distance between nodes
  Vector *m_allocatedPositions; //Allocated positions for all nodes
  int *m_matrix; //Connectivity matrix
  int *m_currentNode; //Current node
};

} // namespace ns3

#endif /* MY_MOBILITY_HELPER_H */
