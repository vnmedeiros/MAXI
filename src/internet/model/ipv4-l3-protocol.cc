...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  InfoTag infoTag; //FAZER: IPV4-L3, esse codigo não deveria está nessa classe e sim dentro da joint-routing.
    if ( packet->PeekPacketTag(infoTag) ) {
  	  packet->RemovePacketTag(infoTag);
  	  infoTag.SetPreviousNodeID(infoTag.GetNodeID());
  	  infoTag.SetNodeId(this->GetNetDevice(0)->GetNode()->GetId());
