...
SpectrumWifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SpectrumWifiPhy")
	...
	.AddTraceSource ("UpdateLinkQuality",
					"UpdateLinkQuality",
					MakeTraceSourceAccessor (&SpectrumWifiPhy::m_tracedUpdateLinkQuality),
					"ns3::SpectrumWifiPhy::UpdateLinkQualityCallback")
  ;
  return tid;
}
...
{
  NS_LOG_FUNCTION (this);
  if (m_rxSpectrumModel && !m_ChannelChanged) // \FAZER bug para multi radio ao alterar o canal não pe calculado o "m_rxSpectrumModel"
    {
...
void SpectrumWifiPhy::NotifyUpdateLinkQuality(Ptr<Packet> packet, int nodeId, double snr, double sinr, double per, bool received) {
	InfoTag it;
	if (packet->PeekPacketTag(it) ) {
		if(TypeTag::TAG_DATA == it.GetTypeTag() ) {
			m_tracedUpdateLinkQuality( it.GetSrc(), nodeId, snr, sinr, per, received);
		} else if (TypeTag::TAG_PRELOAD == it.GetTypeTag() ) {
			m_tracedUpdateLinkQuality( it.GetSrc(), nodeId, snr, sinr, per, received);
		}
	}
}
